...
var _getErrorObject = function(defaultMessage, err) {
  ...
  if (typeof err.error === 'object' && typeof err.error.message === 'string') {
    ...
    errorObject = new ApiError(err.error.message, err.error.status);
  } else if (typeof err.error === 'string') {
    // Authorization Error format
    errorObject = new ApiError(err.error + ': ' + err['error_description']);
  ...
  if (!errorObject) {
    // Unexpected format
    errorObject = new ApiError(defaultMessage + ': ' + JSON.stringify(err));
  }
